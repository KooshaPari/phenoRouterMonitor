...
impl CostCalculation {
    ...
    /// Get cost per 1000 tokens (for comparison)
...
impl BatchCost {
    ...
    /// Get total cost in microdollars
...
mod tests {
    ...
    #[test]
    fn test_cost_calculation_request_new() {
        let req = CostCalculationRequest::new("gpt-4".to_string(), 100, 50);
        assert_eq!(req.model, "gpt-4");
        assert_eq!(req.input_tokens, 100);
        assert_eq!(req.output_tokens, 50);
    }

    #[test]
    fn test_cost_calculation_new() {
        ...
        assert_eq!(calc.total_tokens(), 150);
        assert!((calc.total_cost_usd - 0.00375).abs() < 0.0001);
    }

    #[test]
    fn test_cost_calculation_total_tokens() {
        let calc = CostCalculation::new(
            "gpt-4".to_string(),
            1000,
            500,
        ...
        assert_eq!(calc.total_tokens(), 1500);
    }

    #[test]
    fn test_cost_calculation_cost_per_1k_tokens() {
        let calc = CostCalculation::new(
            "gpt-4".to_string(),
            1000,
            500,
        ...
        let cost_per_1k = calc.cost_per_1k_tokens();
        let expected = (0.0375 / 1500.0) * 1000.0;
        assert!((cost_per_1k - expected).abs() < 0.0001);
    }

    #[test]
    fn test_cost_calculation_cost_ratio() {
        let calc = CostCalculation::new(
            "gpt-4".to_string(),
            1000,
            500,
        ...
        let (input_ratio, output_ratio) = calc.cost_ratio();
        assert!((input_ratio - 40.0).abs() < 0.1); // 0.015 / 0.0375 = 40%
        assert!((output_ratio - 60.0).abs() < 0.1); // 0.0225 / 0.0375 = 60%
    }

    #[test]
    fn test_cost_calculation_microdollars() {
        let calc = CostCalculation::new(
            "gpt-4".to_string(),
            100,
            50,
        ...
        let microdollars = calc.cost_microdollars();
        let expected = (0.00375 * 1_000_000.0).round() as u64;
        assert_eq!(microdollars, expected);
    }
    ...
    fn test_cost_calculator_calculate_from_text() {
        ...
        assert!(calculation.total_cost_usd > 0.0);
    }

    #[test]
    fn test_cost_calculator_batch() {
        let calc = CostCalculator::new();
        let requests = vec![
            CostCalculationRequest::new("gpt-4".to_string(), 1000, 500),
            CostCalculationRequest::new("claude-opus".to_string(), 1000, 500),
        ];
        let batch = calc.calculate_batch(&requests).unwrap();
        assert_eq!(batch.request_count, 2);
        assert!(batch.total_cost_usd > 0.0);
    }

    #[test]
    fn test_batch_cost_average_cost_per_request() {
        let batch = BatchCost {
            calculations: vec![],
            total_cost_usd: 0.01,
        ...
        let avg = batch.average_cost_per_request();
        assert!((avg - 0.005).abs() < 0.0001);
    }

    #[test]
    fn test_batch_cost_total_tokens() {
        let batch = BatchCost {
            calculations: vec![],
            total_cost_usd: 0.01,
            total_input_tokens: 2000,
        ...
        assert_eq!(batch.total_tokens(), 3000);
    }

    #[test]
    fn test_cost_calculator_unsupported_model() {
        let calc = CostCalculator::new();
        assert!(!calc.is_supported("unknown-model-xyz"));
    }
    ...
    fn test_cost_calculation_zero_tokens() {
        ...
        assert_eq!(calc.cost_per_1k_tokens(), 0.0);
    }
}
