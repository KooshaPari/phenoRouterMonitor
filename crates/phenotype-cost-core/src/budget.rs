...
impl Default for BudgetManager {
    fn default() -> Self {
        Self::new(BudgetLimits::small_team())
    }
}
...
mod tests {
    ...
    #[test]
    fn test_budget_limits_new() {
        let limits = BudgetLimits::new(50.0, 1000.0, 5.0).unwrap();
        assert_eq!(limits.daily_limit, 50.0);
        assert_eq!(limits.monthly_limit, 1000.0);
        assert_eq!(limits.per_request_limit, 5.0);
    }

    #[test]
    fn test_budget_limits_invalid_zero() {
        let result = BudgetLimits::new(0.0, 1000.0, 5.0);
        assert!(result.is_err());
    }

    #[test]
    fn test_budget_limits_per_request_exceeds_daily() {
        let result = BudgetLimits::new(50.0, 1000.0, 100.0);
        assert!(result.is_err());
    }

    #[test]
    fn test_budget_limits_presets() {
        let small = BudgetLimits::small_team();
        assert_eq!(small.daily_limit, 50.0);

        let medium = BudgetLimits::medium_team();
        assert_eq!(medium.daily_limit, 200.0);

        let enterprise = BudgetLimits::enterprise();
        assert_eq!(enterprise.daily_limit, 1000.0);
    }

    #[test]
    fn test_budget_usage_new() {
        let usage = BudgetUsage::new();
        assert_eq!(usage.daily_spent, 0.0);
        assert_eq!(usage.monthly_spent, 0.0);
    }

    #[test]
    fn test_budget_usage_add_cost() {
        let mut usage = BudgetUsage::new();
        usage.add_cost(10.0);
        assert_eq!(usage.daily_spent, 10.0);
        assert_eq!(usage.monthly_spent, 10.0);
    }
    ...
    #[test]
    fn test_budget_manager_add_cost() {
        let limits = BudgetLimits::new(50.0, 1000.0, 10.0).unwrap();
        let manager = BudgetManager::new(limits);

        manager.add_cost(5.0);
        let info = manager.info();
        assert_eq!(info.daily_spent, 5.0);
    }
    ...
    #[test]
    fn test_budget_manager_status_healthy() {
        let limits = BudgetLimits::new(50.0, 1000.0, 10.0).unwrap();
        let manager = BudgetManager::new(limits);

        assert_eq!(manager.status(), BudgetStatus::Healthy);
    }
    ...
    #[test]
    fn test_budget_usage_daily_remaining() {
        let mut usage = BudgetUsage::new();
        usage.daily_spent = 30.0;

        let remaining = usage.daily_remaining(50.0);
        assert_eq!(remaining, 20.0);
    }

    #[test]
    fn test_budget_usage_daily_percentage() {
        let mut usage = BudgetUsage::new();
        usage.daily_spent = 25.0;

        let pct = usage.daily_percentage(50.0);
        assert!((pct - 50.0).abs() < 0.01);
    }

    #[test]
    fn test_budget_manager_info() {
        let limits = BudgetLimits::new(50.0, 1000.0, 10.0).unwrap();
        let manager = BudgetManager::new(limits);
        manager.record_cost(15.0).ok();

        let info = manager.info();
        assert_eq!(info.daily_spent, 15.0);
        assert_eq!(info.daily_remaining, 35.0);
    }

    #[test]
    fn test_budget_info_is_healthy() {
        let info = BudgetInfo {
            daily_limit: 50.0,
            daily_spent: 10.0,
            daily_remaining: 40.0,
            daily_percentage: 20.0,
            daily_status: BudgetStatus::Healthy,
            monthly_limit: 1000.0,
            monthly_spent: 100.0,
            monthly_remaining: 900.0,
            monthly_percentage: 10.0,
            monthly_status: BudgetStatus::Healthy,
