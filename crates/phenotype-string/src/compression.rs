...
/// # Example
/// ```
/// use phenotype_string::compression::compress;
...
pub fn compress(input: &[u8], level: i32) -> Result<Vec<u8>> {
    ...
    zstd::encode_all(input, level).map_err(|e| Error::Compression(e.to_string()))
}
...
/// # Example
/// ```
/// use phenotype_string::compression::{compress, decompress};
/// let data = "Hello, World!".as_bytes();
/// let compressed = compress(data, 3).expect("compression failed");
