/// Builder pattern derive macro for structs.
    ...
    match &input.data {
        Data::Struct(data) => {
            ...
            let build_field_assignments = fields.iter().map(|f| {
                let field_name = &f.ident;
                let field_str = field_name.as_ref().map(|n| n.to_string()).unwrap_or_default();
                let err_msg = format!("missing field: {}", field_str);
                quote! {
                    #field_name: self.#field_name.ok_or_else(|| #err_msg.to_string())?,
                }
            });
