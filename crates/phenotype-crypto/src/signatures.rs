...
pub fn compute_hmac(message: &[u8], key: &[u8]) -> Result<Vec<u8>, SignatureError> {
    let mut mac =
        HmacSha256::new_from_slice(key).map_err(|_| SignatureError::VerificationFailed)?;
    mac.update(message);
...
/// ```
pub fn verify_hmac(message: &[u8], signature: &[u8], key: &[u8]) -> Result<(), SignatureError> {
...
pub fn verify_hmac_hex(
    ...
    let signature =
        hex::decode(hex_signature).map_err(|e| SignatureError::HexDecodeError(e.to_string()))?;
