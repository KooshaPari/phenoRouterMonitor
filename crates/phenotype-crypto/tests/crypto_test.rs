...
use phenotype_crypto::{
    blake3_hash, compute_hmac, compute_hmac_hex, content_id, decrypt_aes_gcm, encrypt_aes_gcm,
    sha256_hash, verify_hmac, verify_hmac_hex, HashAlgorithm, Pbkdf2Kdf,
};
...
fn test_end_to_end_secure_storage() {
    ...
    let kdf = Pbkdf2Kdf::new();
    let salt = [99u8; 16];
    let encryption_key = kdf.derive(password, &salt).expect("Key derivation failed");

    // Step 2: Encrypt data
    let encrypted = encrypt_aes_gcm(plaintext, &encryption_key, None).expect("Encryption failed");

    // Step 3: Compute HMAC over encrypted data (for integrity)
    let auth_key = b"auth-secret";
    let signature = compute_hmac(&encrypted, auth_key).expect("HMAC computation failed");
    ...
    // Step 5: Decrypt
    let decrypted = decrypt_aes_gcm(&encrypted, &encryption_key, None).expect("Decryption failed");
    assert_eq!(decrypted, plaintext);
}
...
fn test_hmac_hex_workflow() {
    ...
    let hex_signature = compute_hmac_hex(message, key).expect("Hex HMAC computation failed");
...
fn test_various_key_sizes() {
    ...
    let kdf_fast = Pbkdf2Kdf::with_iterations(10_000).expect("KDF creation failed");
    let kdf_slow = Pbkdf2Kdf::with_iterations(600_000).expect("KDF creation failed");

    let key_fast = kdf_fast.derive(password, &salt).expect("Fast KDF failed");
    let key_slow = kdf_slow.derive(password, &salt).expect("Slow KDF failed");
...
fn test_encryption_with_multiple_aad() {
    ...
    // Decrypt with same AAD
    let decrypted = decrypt_aes_gcm(&encrypted, &key, Some(aad)).expect("Decryption failed");
    assert_eq!(decrypted, plaintext);
...
fn test_large_data_encryption() {
    ...
    let encrypted = encrypt_aes_gcm(&plaintext, &key, None).expect("Large data encryption failed");

    let decrypted = decrypt_aes_gcm(&encrypted, &key, None).expect("Large data decryption failed");
...
fn test_concurrent_encryption() {
    ...
    let encrypted1 = encrypt_aes_gcm(plaintext1, &key1, None).expect("Encryption 1 failed");
    let encrypted2 = encrypt_aes_gcm(plaintext2, &key2, None).expect("Encryption 2 failed");
    ...
    let decrypted1 = decrypt_aes_gcm(&encrypted1, &key1, None).expect("Decryption 1 failed");
    let decrypted2 = decrypt_aes_gcm(&encrypted2, &key2, None).expect("Decryption 2 failed");
