...
pub fn compute_hash(
    ...
    let payload_bytes = serde_json::to_string(payload).map_err(|_| HashError::InvalidHashLength(0))?;
    hasher.update(payload_bytes.as_bytes());
    hasher.update(actor.as_bytes());
    let prev_bytes = decode_hex(prev_hash).map_err(|_| HashError::InvalidHashLength(prev_hash.len()))?;
...
pub fn verify_chain(events: &[(String, String)]) -> Result<(), HashError> {
    ...
    for (i, (_, prev_hash)) in events.iter().enumerate().skip(1) {
        if prev_hash != &events[i - 1].0 {
            return Err(HashError::ChainBroken { sequence: (i + 1) as i64 });
        }
    }
    Ok(())
}
...
mod tests {
    ...
    #[test]
    fn verify_chain_empty() { verify_chain(&[]).unwrap(); }
