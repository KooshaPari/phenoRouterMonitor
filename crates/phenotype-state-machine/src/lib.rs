...
/// Guard function type: (current_state, event) -> bool
type GuardFn = dyn Fn(&str, &str) -> bool + Send + Sync;
/// State callback function type: (new_state) -> ()
type StateCallback = dyn Fn(&str) + Send + Sync;
