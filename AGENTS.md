...
This is the **repos shelf** — a polyrepo containing ~102 independent projects (after consolidation).
...
3. Does the task involve a file path like `AgilePlus/...` or `thegent/...`? Use that project.
4. Otherwise, assume shelf-level work.

### WORKING IN A PROJECT
```
cd <project-name>    # e.g., cd AgilePlus
pwd                   # Always verify
```
...
Good: `feat(AgilePlus): add token refresh with exponential backoff`
...
**Example: thegent satellites**
- thegent-mesh → Separate repo (deleted, merged into thegent)
- thegent-plugin-host → Separate repo (deleted, merged into thegent)
- thegent-shm → Separate repo (deleted, merged into thegent)
- thegent-subprocess → Separate repo (deleted, merged into thegent)
- thegent-metrics → Kept in thegent/crates/ (workspace member)
- thegent-cache → Merged into Stashly

**Status**: Intentionally decomposed for large-scale architecture, now consolidated.

### Template Registry Pattern
Used for: Language-specific scaffolding

**Example: hexagon templates**
- hexagon-go, hexagon-rust, hexagon-zig → Unified into `hexagon/templates/`
- HexaKit/* → Merged into `hexagon/templates/`
- template-lang-* → Merged into `hexagon/templates/`

**Result**: 12+ scattered repos → 1 unified registry

### SDK Extraction Pattern
Used for: Breaking down monolithic SDKs

**Example: phenoSDK**
- phenoSDK/quality → Profila
- phenoSDK/storage → Stashly (merged)
- phenoSDK/tasks → Tasken
- phenoSDK/lib/connection_pool → phenotype-connection

**Status**: In progress - multi-phase extraction
...
|------------|---------|--------|
| agentapi | agentapi-plusplus | ✅ Merged |
| agentapi-deprec | agentapi-plusplus | ✅ Merged |
| agentapi-temp | agentapi-plusplus | ✅ Merged |
| CLIProxyAPI | cliproxyapi-plusplus | ✅ Merged |
| bifrost-extensions | Kogito | ✅ Merged |

---

## Pheno* Ecosystem

**19 active pheno* repositories:**

| Repository | Items | Status |
|------------|-------|--------|
| phenoSDK | 73 | Active - extraction in progress |
| phenoGoKit | 48 | Active |
| phenoConfigTs | 20 | Active |
| phenoGovernance | 14 | Active |
| phenoLoggingZig | 13 | Active |
| phenoMiddlewarePy | 13 | Active |
| phenoCipher | 9 | Active |
| phenoHub | 9 | Active |
| phenoPatch | 9 | Active |
| phenoRouterMonitor | 9 | Active |
| phenoSentinel | 9 | Active |
| phenoSkills | 9 | Active |
| phenoStandards | 9 | Active |
| phenoXdd | 9 | Active |
| phenoXddLib | 9 | Active |
| phenoEvaluation | 5 | Minimal |
| phenoResearchEngine | 5 | Minimal |
| phenoTypes | 5 | Minimal |
| phenoVessel | 5 | Minimal |
