...
## 2026-03-29 - AgilePlus Crate-Specific Duplication Analysis
...
Comprehensive analysis of AgilePlus crates revealing opportunities for libification and consolidation. Identified **~300 LOC** of potential savings through pattern extraction and crate consolidation.

### HTTP Transport Duplication (HIGHEST PRIORITY)

**Duplication Location:** `crates/agileplus-core/src/http/transport.rs` vs `crates/agileplus-agent-impl/src/transport/`

| Aspect | Current State | Savings |
|--------|---------------|---------|
| LOC | ~80-100 per impl | **~60-80 LOC** |
| Pattern | Manual HTTP client setup | Extract to shared crate |
| Maintenance | Duplicated across 2+ crates | Single source of truth |

**Recommended Extraction:**
```
crates/agileplus-http/           (NEW)
├── src/
│   ├── mod.rs                 (50 LOC) - Re-exports
│   ├── client.rs              (120 LOC) - HTTP client traits
│   ├── retry.rs               (80 LOC)  - Retry logic
│   └── middleware.rs          (60 LOC)  - Auth, logging
```

### Health Enum Duplication

**Duplicated Locations:** 4 crates

| Crate | Health Status Enum | LOC | Common Fields |
|-------|-------------------|-----|---------------|
| `agileplus-core` | `HealthStatus` | 15 | `healthy`, `degraded`, `unhealthy` |
| `agileplus-agent-impl` | `AgentHealth` | 18 | `ready`, `busy`, `error` |
| `agileplus-storage` | `StorageHealth` | 12 | `connected`, `syncing`, `error` |
| `agileplus-mcp` | `McpHealth` | 14 | `initialized`, `error` |

**Total:** 59 LOC duplicated across 4 crates

**Recommended Extraction:**
```rust
// crates/agileplus-health/src/lib.rs
...
**Savings:** ~45 LOC (76% reduction)

### Config Loading Patterns

**Duplicated Locations:** 5 crates

| Pattern | Locations | LOC | Canonical Location |
|---------|----------|-----|-------------------|
| TOML parsing | core, agent, storage, mcp | ~40 | `agileplus-config` |
| Env var loading | core, agent | ~15 | `agileplus-config` |
| Config validation | core, agent | ~25 | `agileplus-config` |
| Secret loading | core, storage | ~20 | `agileplus-secrets` |

**Recommended Extraction:**
```
crates/agileplus-config/        (ENHANCE)
├── src/
│   ├── mod.rs                 (100 LOC) - Re-exports
│   ├── toml.rs               (150 LOC) - TOML parsing
│   ├── env.rs                 (50 LOC)  - Env var loading
│   ├── validation.rs          (80 LOC)  - Schema validation
│   └── secrets.rs             (60 LOC)  - Secret loading
```

**Savings:** ~100 LOC (67% reduction)

### Error Enum Consolidation

**36+ Error Variants Found Across Crates**

| Error Category | Variants | Crates | Recommendation |
|---------------|----------|--------|----------------|
| `ConnectionError` | 4 | core, agent, storage | Consolidate |
| `AuthError` | 3 | core, agent, mcp | Consolidate |
| `ParseError` | 5 | core, agent, config | Consolidate |
| `StorageError` | 4 | storage, agent | Consolidate |
| `TimeoutError` | 3 | core, agent | Consolidate |
| `ValidationError` | 4 | core, config | Consolidate |
| `RateLimitError` | 2 | agent, mcp | Consolidate |
| `NotFoundError` | 3 | storage, agent | Consolidate |
| `PermissionError` | 2 | core, storage | Consolidate |
| `ConflictError` | 2 | storage, agent | Consolidate |

**Recommended Extraction:**
```rust
// crates/agileplus-error/src/lib.rs
...
pub enum Error {
    ...
    Parse { context: String, source: ParseError },
...
**Savings:** ~150 LOC (45% reduction)

### Trait Duplication

**Duplicated Traits:** 3+ traits across crates

| Trait | Locations | Methods | Recommendation |
|-------|----------|---------|----------------|
| `Repository` | core, storage | 8 | Consolidate |
| `Cache` | core, agent | 6 | Consolidate |
| `EventBus` | core, agent | 4 | Consolidate |

**Savings:** ~40 LOC (50% reduction)

### CLI Argument Parsing

**Duplication:** 3 CLI tools with similar argument patterns

| Tool | LOC | Overlap |
|------|-----|---------|
| `agileplus-cli` | 200 | ~120 shared |
| `agileplus-mcp` | 150 | ~120 shared |
| `agileplus-agent` | 100 | ~120 shared |

**Recommended Extraction:**
```
crates/agileplus-cli-core/     (NEW)
├── src/
│   ├── mod.rs
│   ├── args.rs              (80 LOC) - Shared arguments
│   ├── config.rs            (60 LOC) - Config commands
│   └── output.rs           (50 LOC) - Output formatting
```

**Savings:** ~80 LOC (40% reduction)

### Async Pattern Duplication

**Found:** 4 instances of similar async retry patterns

```rust
// Pattern duplicated across:
// - core/src/retry.rs
// - agent-impl/src/retry.rs
...
**Savings:** ~90 LOC (75% reduction)

### Total LOC Reduction Opportunities

| Category | LOC | Priority |
|----------|-----|----------|
| HTTP Transport extraction | 60-80 | P0 |
| Config Loading consolidation | 100 | P0 |
| Error Enum consolidation | 150 | P1 |
| Health Enum consolidation | 45 | P1 |
| Trait consolidation | 40 | P1 |
| CLI Argument extraction | 80 | P2 |
| Async Pattern extraction | 90 | P2 |
| **TOTAL** | **~565-585 LOC** | |
...
```
crates/
├── agileplus-core/            (REFACTOR)
├── agileplus-config/          (ENHANCE)
│   └── src/toml.rs, env.rs, validation.rs, secrets.rs
├── agileplus-error/           (NEW - 150 LOC)
│   └── src/lib.rs            - Consolidated error types
├── agileplus-health/         (NEW - 80 LOC)
│   └── src/lib.rs            - Health traits and enums
├── agileplus-http/           (NEW - 200 LOC)
│   └── src/client.rs, retry.rs, middleware.rs
├── agileplus-cli-core/       (NEW - 100 LOC)
│   └── src/args.rs, config.rs, output.rs
└── agileplus-async/          (NEW - 60 LOC)
    └── src/retry.rs, backoff.rs
```

### Implementation Priority

1. **Phase 1 (Immediate):** Create `agileplus-error` crate
   - Consolidate 36+ error variants
   - Estimated: 2 days, saves 150 LOC

2. **Phase 2 (Next Sprint):** Create `agileplus-config` enhancements
   - Extract TOML parsing from 5 crates
   - Estimated: 3 days, saves 100 LOC

3. **Phase 3 (Future):** Create `agileplus-http` crate
   - Extract HTTP transport patterns
   - Estimated: 4 days, saves 60-80 LOC

4. **Phase 4 (Backlog):** Create remaining crates
   - Health, CLI-core, Async patterns
   - Estimated: 5 days total, saves 175 LOC

---

## Action Items

- [ ] Create `agileplus-error` crate with consolidated error types
- [ ] Enhance `agileplus-config` with shared config patterns
- [ ] Create `agileplus-health` crate
- [ ] Create `agileplus-http` crate
- [ ] Create `agileplus-cli-core` crate
- [ ] Create `agileplus-async` crate
- [ ] Update all crates to use new shared crates
- [ ] Remove duplicated patterns after migration

---

## Next Steps

1. Create spec for `agileplus-error` crate in AgilePlus
2. Plan migration path for existing error types
3. Create ADR for new crate structure
