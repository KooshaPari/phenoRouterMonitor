...
use agileplus_domain::{
    domain::{
        audit::AuditEntry,
        feature::Feature,
        governance::{GovernanceContract, PolicyRule},
        metric::Metric,
...
impl StoragePort for SqliteStorageAdapter {
    ...
    async fn create_evidence(&self, ev: &agileplus_domain::domain::governance::Evidence) -> Result<i64, DomainError> {
        let conn = self.lock()?;
        crate::repository::evidence::create_evidence(&conn, ev)
    }

    async fn get_evidence_by_wp(&self, wp_id: i64) -> Result<Vec<agileplus_domain::domain::governance::Evidence>, DomainError> {
        let conn = self.lock()?;
        crate::repository::evidence::get_evidence_by_wp(&conn, wp_id)
    }

    async fn get_evidence_by_fr(&self, fr_id: &str) -> Result<Vec<agileplus_domain::domain::governance::Evidence>, DomainError> {
        let conn = self.lock()?;
