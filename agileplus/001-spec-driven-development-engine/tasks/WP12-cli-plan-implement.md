...
```bash
agileplus implement WP12 --base WP11
```
...
- **Plan**: `agileplus/001-spec-driven-development-engine/plan.md` -- agent dispatch flow, conflict resolution design, scheduler logic
- **Data Model**: `agileplus/001-spec-driven-development-engine/data-model.md` -- WorkPackage entity (file_scope, pr_url, pr_state, agent_id), WpDependency entity, GovernanceContract entity
...
- **Steps**:
  ...
  2. Implement `run_plan(args: PlanArgs, ctx: &AppContext) -> Result<()>`:
     ...
     - Generate WP prompt files under `agileplus/{slug}/tasks/WPnn-title.md`.
     - Create governance contract:
       - Define required evidence for each state transition.
       - Reference applicable policy rules.
       - Persist as `agileplus/{slug}/contracts/governance-v1.json`.
  ...
  2. Implement `build_pr_description(wp: &WorkPackage, feature: &Feature, spec_content: &str) -> String`:
     - Generate a Markdown PR body:
       ...
       **WP ID**: {wp.id} | **Sequence**: {wp.sequence}
       ...
       Full specification: `agileplus/{feature.slug}/spec.md`
       Implementation plan: `agileplus/{feature.slug}/plan.md`
...
2. **Use CLI**: `agileplus agent tasks move-task WP12 --to <lane> --note "message"` (recommended)
