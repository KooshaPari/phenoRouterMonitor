...
```bash
agileplus implement WP07 --base WP05
```
...
4. Artifact operations read/write files in `agileplus/<feature>/` paths.
...
- **Artifact layout**: All spec artifacts live in `agileplus/<feature>/` as defined in `data-model.md` "Git Artifact Layout".
...
**Purpose**: Read and write spec/plan/audit artifacts in the `agileplus/<feature>/` directory structure.
...
2. **read_artifact**:
   ...
   ```
   - Compute full path: `<repo_root>/agileplus/<feature_slug>/<relative_path>`
...
5. **scan_feature_artifacts**:
   ...
   ```
   - Base path: `<repo_root>/agileplus/<feature_slug>/`
...
2. Implement a scanning function:
   ...
   ```
   - List directories under `agileplus/` in the working tree.
...
3. Implement commit history extraction:
   ...
   ```
   - Walk the commit log filtered to paths under `agileplus/<feature_slug>/`.
...
|-----------|--------|-------|---------|
| 2026-02-27T00:00:00Z | Prompt generated | system | WP07 prompt created via /agileplus.tasks |
